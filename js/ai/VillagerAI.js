...
export class VillagerAI {
    ...
    async handlePlayerChat(villager, playerInput) {
        ...
        if (result) {
            ...
            if (result.moodChange) {
                villager.mood = Math.max(0, Math.min(20, villager.mood + result.moodChange));
            }
        ...
        if (fallback.moodChange) {
            villager.mood = Math.max(0, Math.min(20, villager.mood + fallback.moodChange));
        }
    ...
    generateVillagerPrompt(ctx) {
        ...
        // 根据心情生成状态描述
        const moodDesc = ctx.mood >= 16 ? '心情很好' :
                         ctx.mood >= 12 ? '心情不错' :
                         ctx.mood >= 8 ? '心情一般' :
                         ctx.mood >= 4 ? '心情不太好' : '心情很糟糕';
        const staminaDesc = ctx.stamina >= 40 ? '精力充沛' :
                           ctx.stamina >= 25 ? '体力还行' :
                           ctx.stamina >= 10 ? '有点累了' : '快没力气了';
...
【你现在的状态】
${staminaDesc}（体力${ctx.stamina}/${ctx.maxStamina}），${moodDesc}（心情${ctx.mood}/20）
    ...
    getFallbackResponse(villager, playerInput) {
        ...
        if (/你好|嗨|早上好|下午好|晚上好|在吗|hi|hello/.test(input)) {
            return this._buildFallback(villager, playerInput, [
                `村长好呀！今天${villager.mood >= 12 ? '心情不错' : '有点累'}~`,
                `哟，村长！我正${action}呢~`,
                `嘿，村长来啦！有什么事吗？`,
        ...
        if (/计划|打算|安排|在做|忙什么|干什么|干嘛|任务/.test(input)) {
            return this._buildFallback(villager, playerInput, [
                `我现在在${action}。${villager.stamina > 25 ? '体力还充足，还能接着干！' : '不过有点累了...'}`,
                `今天安排得挺满的，正在${action}呢。`,
                `我吗？在${action}呀~${villager.mood >= 12 ? '干得挺开心的！' : '还行吧...'}`,
            ], 'neutral', [{ text: '辛苦了，注意休息', tone: 'positive' }, { text: '还有什么想做的？', tone: 'neutral' }]);
        }

        // 关于心情
        if (/心情|开心|难过|高兴|怎么了|还好吗/.test(input)) {
            const moodReply = villager.mood >= 14 ? '挺好的！谢谢村长关心~' :
                             villager.mood >= 8 ? '还行吧，一般般。' : '说实话...有点不太开心...';
        ...
        return this._buildFallback(villager, playerInput, [
            ...
            `好的村长，我${villager.mood >= 12 ? '知道了~' : '尽量吧...'}`,
